...
const BootstrapIcons = ({}) => {
	...
	const bootstrapIconsArr = Object.keys(bootstrapIcons)
	...
	const onFilter = async(evt) =>{
		delay(()=>{
			...
			const qRegex = new RegExp(`${query}`)
			// console.log(query)
			HSClipboard.collection = []
			setIcons(bootstrapIconsArr.filter(icon => icon.match(qRegex)))
			setTimeout(()=>{
				HSClipboard.init('.js-clipboard')
			},3000)
		})
	...
	return(<>
		...
		<div className="sm:inline-flex sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 w-full">
		  <label htmlFor="inline-input-label-with-helper-text" class="block text-sm font-medium mb-2 dark:text-white">Filter</label>
			...
			<div className={`w-full columns-1 overflow-x-hidden`}>
				{
					icons.map((icon,index)=>{
						...
						return(<div className="hs-tooltip inline-block  [--trigger:click] [--placement:right]" key={index}>
