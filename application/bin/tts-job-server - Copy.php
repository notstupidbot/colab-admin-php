...
function pre_main($ci, $job_id, $url, $output_file, $proxy=""){
...
function info($ci, $job_id, $output){
    $job = $ci->m_job->getById($job_id);
    if(!empty($job)){
        $subscriber_id = $job['subscriber_id'];
        $message = "job id $job_id output";
...
function main(){
    ...
    // ob_start();
    $result = pre_main($ci, $job_id, $url, $output_file, $tts_server_proxy);
    $result['url'] = $url;
