...
export default function ProjectItem({config, project, page}){
	...
	const updateList = async(page_number)=>{
		...
		const res = await Prx.get(`${config.getApiEndpoint()}/api/tts/sentence?page=${page_number}&limit=${grid.limit}&order_by=${grid.order_by}&order_dir=${grid.order_dir}&project_id=${project_id}`);
		try{
			setGrid(res.data)
		}catch(e){
